...
describe("typed desktop client boundary", () => {
  ...
  it("validates responses before rendering and rejects unsupported verification claims", async () => {
    ...
    await expect(client.call("computer.inspect", { workspaceId: "test-workspace" })).rejects.toThrow("invalid response");
  });
  it("rejects extra settings and request fields before IPC", async () => {
    ...
    await expect(client.call("settings.update", {
      workspaceId: "test-workspace", ...testWorkspace().settings, appearance: { theme: "dark", density: "comfortable", secret: "not allowed" },
  ...
  it("unsubscribes listeners and ignores malformed desktop events", async () => {
    ...
    const changed = vi.fn();
    const remove = await client.subscribe("test-workspace", { area: "work" }, changed);
    ...
    expect(bridge.request).toHaveBeenLastCalledWith({ method: "events.unsubscribe", params: { workspaceId: "test-workspace", subscriptionId } });
  });
  it("cleans listeners when subscription setup fails", async () => {
    const { client, listeners } = setup({});
    await expect(client.subscribe("test-workspace", { area: "work" }, () => {})).rejects.toThrow();
