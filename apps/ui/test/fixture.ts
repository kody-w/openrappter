...
export const testAgent = {
  ...
  approvalPolicy: "always" as const, enabled: true, updatedAt: timestamp,
};
export function testWorkspace() {
  return snapshotSchema.parse({
    workspaceId: "test-workspace", revision: 0, agents: [], tasks: [], runs: [], approvals: [], artifacts: [], automations: [],
    settings: {
...
export class FixtureClient implements WorkClient {
  ...
  async call<M extends RpcMethod>(method: M, params: RpcInput<M>): Promise<RpcResult<M>> {
    ...
    switch (method) {
      ...
      case "providers.list": result = this.status.ready ? [{ id: "test-provider", name: "Injected provider", configured: true, models: ["test-model"], detail: "Test service." }] : []; break;
      ...
      case "work.createTask": {
        ...
        const task = { ...rest, id: requestId, state: "queued" as const, createdAt: now, updatedAt: now };
      ...
      case "work.assignTask": {
        ...
        task.agentId = String(object.agentId); result = task; break;
      }
      case "runs.start": {
        if (!this.status.ready) throw new Error("Runtime is not configured.");
        const task = this.workspace.tasks.find((task) => task.id === object.id)!;
        const run = {
          id: crypto.randomUUID(), taskId: task.id, agentId: task.agentId!, state: "running" as const,
      ...
      case "automations.save": {
        ...
        const automation = { ...input, updatedAt: now, nextRunAt: input.enabled ? now : null };
...
export function populatedClient() {
  ...
  client.workspace.tasks.push({
    ...
    agentId: testAgent.id, priority: "high", state: "awaiting_approval", createdAt: timestamp, updatedAt: timestamp,
  });
  client.workspace.runs.push({
    id: "test-run", taskId: "test-task", agentId: testAgent.id, state: "awaiting_approval",
