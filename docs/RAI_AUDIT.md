...
| **Default approval policy is `allowlist`, but the default allowlist includes `curl`, `wget`, `chmod`, `pip`, `npm`** — these are dual-use (arbitrary download+execute) | **High** | Ship a *stricter* default; move network+install binaries behind explicit approval; make the permissive set opt-in |
