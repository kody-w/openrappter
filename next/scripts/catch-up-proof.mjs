...
const normal = await hosted.ai.catchUp(bot.root, grant.capability, { from: null });
assert(normal.steps.every(step => step.guest === null));
const privatePage = await hosted.ai.catchUp(bot.root, grant.capability, { from: null, guest: guest.option });
...
const player = new CatchUpPlayer(bot.root, 'root');
player.load(privatePage, privatePage.timelineDigest);
...
const check = spawnSync('python3', [path.join(rootDirectory, 'scripts/check-replay-digests.py'), path.join(evidence, 'replay.json'),
  options.directory, sourceDirectory], { encoding: 'utf8', env: { ...process.env, PYTHONDONTWRITEBYTECODE: '1' } });
