...
export class RecurringWork {
  ...
  async tick(root: string, routineId: string, occurrence: string): Promise<JsonObject> {
    ...
    return this.bots.repository.transaction(async transaction => {
      ...
      const duplicate = memoryFrames(snapshot).find(f => f.payload.operationId === id);
      ...
      const context = memoryFrames(snapshot).filter(f => permitted.has(String(f.payload.scope))).slice(-8);
      const summary = `Canonical recap for ${routine.instruction}: ${context.length} recent scoped public records read first. `
        + 'Source work remains in the referenced canonical occurrences, not copied into this recap. '
      ...
      const frame = await this.bots.appendEvent(transaction, snapshot, 'routine.tick',
        { routineId, occurrence, summary, evidence: [...new Set([routine.source, ...context.map(f => f.frame_hash)])] }, id, routine.scope);
