...
`commit(capability, command, effect, {signal, expectedHeads?})`:
...
multiple scopes must also validate each referenced scope and inventory; the
host Twin additionally rejects intervening writes during model drafting.
