"""Load and execute the byte-identical RAPP/1 §7.10 conformance deck."""
...
def _vector_root() -> Path:
    ...
    if packaged.is_dir():
        return packaged
    raise FileNotFoundError("vendored RAPP/1 §7.10 RAPPID card vectors are unavailable")
...
def build_rappid_card_fixture(name: str) -> RappidCardFixture:
    ...
    if vector is None:
        raise ValueError(f"unknown RAPP/1 §7.10 RAPPID card scenario: {name}")
