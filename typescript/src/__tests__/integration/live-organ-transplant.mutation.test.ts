...
function makeFlightEvents(
  ...
  return [
    ...
    makeFlightEvent(3, "agent.import.authorization.started", "started", {
      parentId: root,
      source: "agent-import-authorizer",
      metadata: { nonce, mode: "caller-supplied" },
    }),
    makeFlightEvent(4, "gateway.agent.import.started", "started", {
      parentId: root,
      source: "gateway",
      metadata: {
        method: "POST",
    ...
    makeFlightEvent(5, "agent.import.started", "started", {
    ...
    independentProbe: {
      ...
      flight: {
        ...
        causalStepIds: [
          ...
          "import-authorization-completed",
          "demo-completed",
          "trace-completed",
        ],
      },
  ...
  mutation(
    ...
    (input) => {
      mutateExactCommandEvent(
        input,
        (event) =>
          event.kind === "agent.import.started" && event.sequence === 5,
        (event) => {
          const { contentHash: _hash, ...body } = event;
          return {
            ...body,
