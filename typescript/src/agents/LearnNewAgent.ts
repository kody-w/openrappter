 ...
 *
 * Order matters and used to be the other way round. The previous code escaped
 * quotes and then sliced the result at 200 characters, which can cut between a
 * backslash and the character it escapes. The generated file then ends
 *
 *     description: "AAAA\\",
 *
 * where the trailing backslash escapes the closing quote, and the agent will
 * not parse at all. Truncating first means the escaping is applied to final
 * text and can never be sliced in half.
 *
 * `JSON.stringify` also covers what hand-rolled escaping missed: backslashes,
 * control characters, and line separators. A description ending in a single
 * backslash produced a file that could not be parsed.
 */
export function jsStringLiteral(value: string, maxLength?: number): string {
  const text = maxLength === undefined
    ? value
...
/**
 * Make text safe to sit inside a block comment.
 *
 * `*\/` inside the description closed the comment early and everything after
 * it became executable code in the generated agent. This was reachable from an
 * ordinary "create an agent that ..." request:
 *
 *     description: "x *\/ globalThis.INJECTED = 'yes'; /* y"
 *
 * The generated file parsed cleanly and ran the statement on load.
 */
export function docCommentSafe(value: string): string {
  return value
    .replace(/\*\//g, '*\\/')
    .replace(/\r?\n/g, ' ')
...
Generate ONLY the code. No markdown fences. No explanation.`;
  ...
  private generateAgentCodeTemplate(description: string, name: string, className: string): string {
    ...
    for (const [key, param] of Object.entries(extraParams)) {
      ...
      extraParamsStr += `,\n            ${key}: { type: ${jsStringLiteral(p.type)}, description: ${jsStringLiteral(p.description)} }`;
    }
