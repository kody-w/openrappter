...
export function registerRappidCardCommand(program: Command): void {
  ...
  command
    ...
    .description('Export the vendored RAPP/1 §7.10 conformance deck and QR artifacts')
  ...
  command
    .command('verify <card>')
    .description('Verify one vendored scenario at its exact ordered RAPP/1 §7.10 step')
    .requiredOption('--link <uri-or-file>', 'Exact compact URI or link file')
    .option('--scenario <name>', 'RAPP/1 §7.10 mandatory scenario')
    ...
    .action(async (
      ...
      if (options.scenario) {
        if (!RAPPID_CARD_FIXTURE_NAMES.some((name) => name === options.scenario)) {
          throw new Error(`unknown RAPP/1 §7.10 scenario: ${options.scenario}`);
        }
  ...
  command
    ...
    .description('Run one vendored mandatory RAPP/1 §7.10 scenario')
    .requiredOption('--state <sqlite>', 'Durable SQLite nonce/sequence state')
    .action(async (scenario: string, options: { state: string }) => {
      if (!RAPPID_CARD_FIXTURE_NAMES.some((name) => name === scenario)) {
        throw new Error(`unknown RAPP/1 §7.10 scenario: ${scenario}`);
      }
