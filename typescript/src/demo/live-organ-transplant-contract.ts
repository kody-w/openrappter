...
export const REQUIRED_TRANSPLANT_CAUSAL_STEPS = [
  ...
  {
    id: "import-authorization-started",
  ...
  {
    id: "import-authorization-completed",
...
export function evaluateTransplantCausalTrace(
  ...
  const authorizationStarts = byKind("agent.import.authorization.started");
  const authorizationCompleted = byKind(
  ...
  fail(
    authorizationStarts.length === 1,
    "expected exactly one agent.import.authorization.started",
  );
  fail(
    authorizationCompleted.length === 1,
    "expected exactly one agent.import.authorization.completed",
  );
  fail(
  ...
  const semantic: Array<{
    ...
    {
      id: "import-authorization-completed",
      event: authorizationCompleted[0],
    },
  ...
  fail(
    requireRootParent(demoStarts[0]) &&
      requireRootParent(authorizationStarts[0]) &&
      ...
      requireRootParent(authorizationCompleted[0]) &&
  ...
  fail(
    metadataString(authorizationStarts[0], "nonce") === expected.nonce &&
      metadataString(authorizationCompleted[0], "nonce") === expected.nonce &&
      authorizationStarts[0]?.source === "agent-import-authorizer" &&
      authorizationCompleted[0]?.source === "agent-import-authorizer",
    "import authorization boundary mismatch",
  );
