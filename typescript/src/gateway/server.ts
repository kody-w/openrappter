...
function parseAgentImportProvenance(
  ...
  if (present.length === 0) return { kind: 'automatic' };
...
function validateActiveImportTrace(
  ...
  if (boundaries.length !== 1) {
    return {
      ok: false,
      error:
        'The supplied traceId must contain exactly one active import authorization boundary.',
    };
  }
  const boundary = boundaries[0];
  if (
    boundary.source !== 'agent-import-authorizer'
    || boundary.status !== 'started'
  ) {
    return {
      ok: false,
      error: 'The supplied traceId has an invalid import authorization boundary.',
    };
  }
  ...
  if (
    events.some((event) =>
      (
        event.kind === 'agent.import.authorization.completed'
        || event.kind === 'agent.import.authorization.failed'
      )
      && event.parentId === root.id,
    )
  ) {
    return {
      ok: false,
      error: 'The supplied import authorization is no longer active.',
    };
  }
  return { ok: true, root };
}
      ...
      req.on('end', async () => {
        ...
        try {
          ...
          if (pathOnly === '/agents/import') {
            ...
            const importWithProvenance = async (
              provenanceValue: Pick<
                AgentImportProvenance,
                'traceId' | 'scenarioNonce' | 'requestId'
              >,
            ...
            try {
              traceEvents = await recorder.query({
                traceId: provenanceValue.traceId,
                order: 'asc',
              });
            } catch {
              return {
              ...
              async () => {
                const started = await recordFlightEventDurably(recorder, {
                  ...
                  metadata: {
                    ...
                    authMode,
                    filename,
                    nonce: provenanceValue.scenarioNonce,
                    requestId: provenanceValue.requestId,
                ...
                try {
                  result = await recorder.withParent(started.id, () =>
                    runWithAgentImportProvenance(
                      {
                        traceId: provenanceValue.traceId,
                        scenarioNonce: provenanceValue.scenarioNonce,
                        requestId: provenanceValue.requestId,
                        candidateSourceSha256,
            ...
            if (provenance.kind === 'automatic') {
              ...
              if (!recorderHealth.enabled || !recorderHealth.initialized) {
                writeJsonResponse(
                  ...
                  503,
                  {
                    status: 'error',
                    error:
                      'Privileged imports require an available Flight Recorder.',
                  },
                  corsHeaders,
                );
                return;
              ...
              const automaticProvenance = {
                traceId: `agent-import-${randomUUID()}`,
