...
function statePath(name: string): string {
  const path = join(process.cwd(), `.protocol-card-${name}-${process.pid}.sqlite`);
  generated.push(path);
  return path;
}
...
describe('RAPP/1 §7.10 contract drift', () => {
  ...
  it('matches RAPP/1 §7.10 container-only depth at 63, 64, and 65 arrays and objects', () => {
...
describe('RAPP/1 §7.10 mandatory deck', () => {
...
describe('detached JWS, content roots, and durable state', () => {
  ...
  it('accepts only a mode-0600 independently provisioned trust config', async () => {
    const deck = loadRappidCardDeck();
    const path = join(process.cwd(), `.protocol-trust-${process.pid}.json`);
    generated.push(path);
